...
function drawGorillaThoughtBubbles(player) {
  if (state.phase === 'aiming') {
    const currentPlayerIsComputer =
      (settings.numberOfPlayers === 0 && state.currentPlayer === 1 && player === 1) ||
      (settings.numberOfPlayers !== 2 && state.currentPlayer === 2 && player === 2)
  ...
  for (let i = 0; i < hitDetectionPrecision; i++) {
    ...
    if (miss) {
      ...
      const computerThrowsNext =
        settings.numberOfPlayers === 0 || (settings.numberOfPlayers === 1 && state.currentPlayer === 2)
